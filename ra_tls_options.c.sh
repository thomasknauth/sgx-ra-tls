...
if ( [[ ! -z "$LINKABLE_MODE" ]] ); then
    case $LINKABLE_MODE in
        [Yy]* ) QUOTE_TYPE="SGX_LINKABLE_SIGNATURE"; break;;
        [Nn]* ) QUOTE_TYPE="SGX_UNLINKABLE_SIGNATURE"; break;;
        * ) echo "\"$LINKABLE_MODE\" is not acceptable for LINKABLE_MODE, Please answer yes or no."; exit 1;;
    esac
fi
